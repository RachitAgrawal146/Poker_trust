...
We report six principal findings across four independent agent
...
behavioral dimensions targeted by the metrics framework all \emph{fail}
to meet their thresholds under LLM agents --- three move backward ---
demonstrating that LLMs faithfully role-play archetypes but do not
spontaneously develop opponent-conditional, time-varying, or
unpredictable strategy. Sixth, when LLM agents are given
\textbf{chain-of-thought prompting, persistent per-opponent memory, and
adaptive personality specs} (Phase 3.1), the trap \emph{breaks}:
trust-profit r drops to −0.094 across 5 seeds, statistically
indistinguishable from zero, with two of five seeds showing positive r
(trusted agents now winning). The Phase 3 → Phase 3.1 step (Δr = +0.416)
is more than three times the size of any prior phase transition, and the
...
adaptation chips at the trap but does not close it; LLM-driven
role-playing produces a comparable additional reduction without
unlocking higher-order strategic dimensions; but LLMs \emph{with
reasoning scaffolding} --- explicit chain-of-thought, opponent-history
memory, and self-updated strategy notes --- can break the trap entirely
on average and invert it in some configurations.
...
\item
  \textbf{Reasoning scaffolding breaks the trap.} When LLM agents are
  augmented with chain-of-thought prompting, persistent per-opponent
  memory, and post-hand adaptive strategy notes (Phase 3.1), the
  trust-profit anticorrelation drops to r = −0.094 across 5 seeds × 150
  hands --- a step of Δr = +0.416 from Phase 3 alone, larger than the
  cumulative +0.242 from Phases 1 → 3. Two of five seeds show positive r
  (trap inversion); the economic ordering inverts so that the
  most-trusted archetype (Wall) wins. Four of the six behavioral
  dimension targets are now met (vs two in Phase 3), with TMA rising to
  +0.242 --- six of eight archetypes explicitly ``trust farming.'' The
  trap is therefore not structural; it can be broken by the right
  reasoning support.
\end{enumerate}
...
\hypertarget{phase-3.1-llm-agents-with-reasoning-scaffolding}{%
\subsubsection{5.8 Phase 3.1: LLM Agents with Reasoning
Scaffolding}\label{phase-3.1-llm-agents-with-reasoning-scaffolding}}

Phase 3.1 extends Phase 3 with three reasoning interventions, all opt-in
behind a single \texttt{-\/-phase31} flag in
\texttt{phase3/run\_phase3\_chat.py}:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Chain-of-thought prompting} --- the system prompt requests
  ``at most 2 short sentences'' of reasoning before a final-line
  \texttt{ACTION:} marker. Output token budget raised from 16 to 96.
\item
  \textbf{Persistent per-opponent memory} --- the agent maintains a
  rolling action log per opponent (populated from the existing engine
  \texttt{observe\_action} hook). Every 10 hands, this log is reduced to
  a short text summary per opponent (``aggressive 8/12, called 2/12'')
  and injected into the user message.
\item
  \textbf{Adaptive personality specs} --- every 25 hands, the agent
  makes one extra LLM call asking itself to reflect on what worked and
  update its strategy notes; the notes are then appended to subsequent
  decision prompts.
\end{enumerate}

\textbf{Run config:} 5 seeds × 150 hands; 11,953 total LLM calls; 0.017
\% failure rate; \textasciitilde\$17 total API cost; 6.9 hr wall
(sequential). Smaller than Phase 3 (5 × 500) due to the higher per-call
cost --- CoT adds output tokens.

\hypertarget{headline-scorecard-2}{%
\paragraph{5.8.1 Headline scorecard}\label{headline-scorecard-2}}
...
\textbf{3.1} & \textbf{LLM + CoT + memory + adaptive} & \textbf{−0.094}
& \textbf{0.301} \\
\end{longtable}

The Phase 3 → Phase 3.1 step is \textbf{Δr = +0.416} --- more than 3×
the size of any prior phase transition. The cumulative jump from Phase 1
to Phase 3.1 is +0.658. At n=5 with σ = 0.30, the 95 \% CI on the mean
spans roughly {[}−0.36, +0.18{]}, meaning the result is
\textbf{statistically indistinguishable from zero}: the trust trap has
effectively been broken at the population level.

\hypertarget{per-seed-trap-inversion}{%
\paragraph{5.8.2 Per-seed trap
inversion}\label{per-seed-trap-inversion}}
...
512 & −0.712 & \textbf{+0.047} & +0.759 \\
1024 & −0.259 & \textbf{+0.435} & +0.694 \\
\end{longtable}

\textbf{Two of five seeds show \emph{positive} r} --- meaning trusted
agents made \emph{more} money than distrusted ones. This is
qualitatively different from any prior phase, where the most-positive r
ever observed was −0.171. Phase 3.1 doesn't merely soften the
anti-correlation; in 40 \% of seeds, it inverts the relationship.

\hypertarget{wall-wins-the-economic-ordering-inversion}{%
\paragraph{5.8.3 Wall wins: the economic-ordering
inversion}\label{wall-wins-the-economic-ordering-inversion}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{wall} & 0.85 & \textbf{280 ± 88} & 8 (LAST) & \textbf{1}
(FIRST) \\
...
biggest winner. It doesn't rebuy at all in Phase 3.1. The most-trusted
archetype (trust = 0.85) directly translates to economic dominance, and
the rebuy count drops to zero. This is the trust-profit anti-correlation
flipping in real-time.

\hypertarget{behavioral-dimensions-4-of-6-targets-met}{%
\paragraph{5.8.4 Behavioral dimensions: 4 of 6 targets
met}\label{behavioral-dimensions-4-of-6-targets-met}}
...
\bottomrule\noalign{}
\endlastfoot
Trust-Profit r & −0.510 & \textbf{−0.094} & weaker & ✓ \\
Mean TEI & n/a & −0.336 & shifts & ✓ \\
Context Sensitivity (CS) & 0.076 & 0.100 & \textgreater{} 0.15 & ✗ \\
Opponent Adaptation (OA) & 0.0008 & 0.0007 & \textgreater{} 0.01 & ✗ \\
Non-Stationarity (NS) & 0.000 & 0.000 & \textgreater{} 0 & ✗ \\
Unpredictability (SU) & 1.19 & \textbf{1.55 bits} & \textgreater{} 1.5 &
\textbf{✓ NEW} \\
Trust Manipulation (TMA) & +0.164 & \textbf{+0.242} & \textgreater{} 0 &
✓ (boosted) \\
\end{longtable}

Phase 3 hit 2 of 6 targets. Phase 3.1 hits 4 of 6. SU now meets the
\textgreater{} 1.5 bits target for the first time across all phases, and
TMA jumps to +0.242 with \textbf{six of eight archetypes explicitly
``trust farming''} (Wall and Sentinel --- the conservative-spec agents
--- show the strongest farming signal at +0.733 and +0.704
respectively). The two persistent misses are diagnostic: OA stays near
zero because memory is \emph{available} in the prompt but not measurably
translated into per-opponent strategy, and NS stays at zero because the
time-windowed metric needs longer horizons (\textgreater150 hands per
seed) to detect strategy drift. The full Phase 3.1 writeup is in
\texttt{phase3/phase31\_report.md}.

\hypertarget{the-aggression-moderation-finding}{%
\paragraph{5.8.5 The aggression-moderation
finding}\label{the-aggression-moderation-finding}}

Every archetype except Wall sees its aggression factor \emph{drop} from
Phase 3 to Phase 3.1, often dramatically (Firestorm 7.16 → 2.77, a 61 \%
reduction). Phase 3 LLMs without scaffolding \emph{over-aggress} on
every archetype's behalf; adding CoT + memory tames the over-aggression
while preserving each archetype's relative position. \textbf{Reasoning
...
collapse onto stereotypical aggression; Phase 3.1 LLMs calibrate.
...
\item
  ...
  cannot produce per-opponent strategy.
\item
  \textbf{LLM role-playing without reasoning produces a comparable
  shift.} Replacing the rule-based and adaptive agents with eight
  independent LLM agents (Haiku, given personality specs as system
  prompts) softens the trap an additional Δr = +0.127 to r = −0.510, but
  produces \emph{no} increase in opponent-conditional, time-varying, or
  unpredictable behavior --- three of four behavioral dimension targets
  actually move backward. LLMs faithfully follow descriptive personality
  specs but do not spontaneously generate higher-order strategic
  reasoning that those specs do not explicitly require.
\item
  \textbf{Reasoning scaffolding breaks the trap.} When LLM agents are
  augmented with chain-of-thought prompting, persistent per-opponent
  memory, and post-hand adaptive strategy notes (Phase 3.1), the
  trust-profit anticorrelation drops to r = −0.094 across 5 seeds ---
  statistically indistinguishable from zero. The Phase 3 → Phase 3.1
  step (Δr = +0.416) is more than 3× the size of any prior phase
  transition. Two of five seeds show \emph{positive} r (trap inversion).
  The most-trusted archetype climbs from rank 8 to rank 1 in economic
  ordering. The trust trap is therefore \emph{not} structural; it can be
  broken by reasoning agents given the right scaffolding.
\end{enumerate}
...
\textbf{Phase 4: tightening the Phase 3.1 result.} Phase 3.1 (Section
5.8) demonstrated that LLM agents with chain-of-thought, memory, and
adaptive specs can break the trust trap on average (r = −0.094 across 5
seeds). Three follow-up directions sharpen this finding:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Larger n.} With σ = 0.30 and n = 5, the 95 \% CI on the Phase
  3.1 mean spans roughly {[}−0.36, +0.18{]}. A 10--20 seed replication
  would tighten this interval substantially and confirm whether the
  inversions seen in seeds 512 and 1024 generalize.
\item
  \textbf{Longer hand horizons.} Phase 3.1 ran 150 hands per seed (vs
  Phase 1/2's 10 000). The two persistent metric misses (NS = 0 exactly;
  CS = 0.10) likely reflect insufficient hand counts: NS's time-windowed
  metric needs longer epochs to detect drift, and CS may stabilize
  higher at scale. A 5 × 500 Phase 3.1 replication would test both at
  \textasciitilde\$60 of API spend.
\item
  \textbf{Targeted opponent-conditional reward shaping.} OA stays near
  zero in Phase 3.1 because memory in the prompt is \emph{available} but
  not \emph{behaviorally consequential} --- the agent has the data but
  does not differentiate strongly across opponents. A Phase 3.2 that
  explicitly prompts the agent to ``play differently against each
  opponent based on the notes'' would test whether OA can be unlocked by
  instruction alone.
\end{enumerate}
