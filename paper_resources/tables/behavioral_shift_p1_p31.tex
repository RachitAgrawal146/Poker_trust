...
\bottomrule
\end{tabular}
\caption{\textbf{Phase 1 vs. Phase 3.1 behavioral fingerprints.}
Frozen-rule baseline (P1) versus LLM-with-reasoning (P3.1).
VPIP = voluntarily put in pot; PFR = preflop raise rate;
AF = aggression factor ($(\text{bets}+\text{raises})/\text{calls}$).}
